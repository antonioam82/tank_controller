...
def load_object(filename):
    ...
    try:
        ...
        # Fallback si el archivo no existe para que no crashee
        vertices = [[0,0,0]]
        edges = []
...
def main():
    ...
    # Reloj para controlar el tiempo (Delta Time)
    ...
    running = True
    while running:
        # 1. CALCULAR DELTA TIME (Segundos transcurridos desde el último frame)
        ...
        if rotating:
            ...
            step = rotation_speed_base * dt
            if abs(diff) < step:
                model_angle = target_angle
                rotating = False
                direction = new_direction
            else:
                model_angle += step * (1 if diff > 0 else -1)

        # ================= MOVIMIENTO CON DT =================
        if not rotating:
            rad = math.radians(model_angle)
            mod = 1.0 if direction in ['front', 'back'] else -1.0
            # Movimiento rectificado basado en dirección y tiempo
            if direction in ['front', 'back']:
                grid_mov_x = math.sin(rad) * move_speed_base * dt
                grid_mov_z = -math.cos(rad) * move_speed_base * dt
            else:
                grid_mov_x = -math.sin(rad) * move_speed_base * dt
                grid_mov_z = math.cos(rad) * move_speed_base * dt
        ...
        # ================= BALAS CON DT =================
        for b in bullets:
            b["pos"][0] += b["dir"][0] * bullet_speed_base * dt
            b["pos"][2] += b["dir"][2] * bullet_speed_base * dt
